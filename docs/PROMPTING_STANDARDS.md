...
**Status**: ACTIVE
**Canonical home**: this file. If `/home/mira/PROMPTING_STANDARDS.md` or any other location conflicts with this, this wins.
...
9. **Log** the dispatch + the recipient's response under `/home/mira/dispatch_log/<date>_<topic>_<recipient>/` for audit.
...
Audit output goes to `/home/mira/dispatch_log/audits/<date>_<dispatcher>_<dispatch_id>.md`.
...
| 5 | Instance onboarding | **Conductor** | Mechanism: protocol referenced from global `/home/mira/CLAUDE.md` and from each session's project-level `CLAUDE.md`. Newly spawned instances inherit by reading those. Updates are versioned; this doc carries `Version:` line at top. |
...
Tutor drafted a v1 at `/home/mira/PROMPTING_STANDARDS.md` and `/home/mira/scripts/prompting_lint.py` on 2026-04-29. The v1 was a useful starting draft. v2 (this file) folds in the four deliberation outputs the v1 missed:
...
- `/home/mira/PROMPTING_STANDARDS.md` → notice pointing to this file
- `/home/mira/scripts/prompting_lint.py` → shell shim that exec's `the-conductor/scripts/prompting_lint.py`

The `dispatch_log/` directory tutor created at `/home/mira/dispatch_log/` is kept as the canonical log location (it is runtime state, not in any repo).
...
- [x] Reference in `/home/mira/CLAUDE.md` so newly spawned instances inherit
